...
describe("IssueUtil ", () => {
  it("getProject method should return Id not present error when no id is sent to the method", async () => {
    let id
    const response = await IssueUtil.getProject(id)
  ...
  it("getIssue method should return startAt must be a valid number error when an invalid startAt value is sent to the method", async () => {
    const project = 'MA', orderBy='key', startAt='ma', maxResult='1'
    const response = await IssueUtil.getIssue(project, orderBy, startAt, maxResult)
    expect(response.data).toBe("startAt must be a valid number");
  });

  it("getIssue method should return maxResult must be a valid number when an invalid maxResult value is sent to the method", async () => {
    const project = 'MA', orderBy='key', startAt='0', maxResult='ma'
    const response = await IssueUtil.getIssue(project, orderBy, startAt, maxResult)
    expect(response.data).toBe("maxResult must be a valid number");
  });

  it("getIssue method should return special character error when project with special characters other than - is sent to the method", async () => {
  ...
  it("getIssue method should return success response when valid parameters are passed to the method", async () => {
